...
if ( ! class_exists( 'WaterWooPDF' ) ) :
	...
	class WaterWooPDF {
		...
		private function plugin_classes() {
			...
			return array(
				...
				'wwpdf_download_handler_22'		=> WWPDF_PATH . 'classes/class_wwpdf_download_handler_22.php',
				'wwpdf_download_handler_23'		=> WWPDF_PATH . 'classes/class_wwpdf_download_handler_23.php',
				'wwpdf_file_handler'		    => WWPDF_PATH . 'classes/class_wwpdf_file_handler.php',
			);
		...
		public function load_hooks() {
			...
			if ( class_exists( 'WC_Download_Handler' ) ) {
				...
				if ( isset( $_GET['download_file'] ) && isset( $_GET['order'] ) && isset( $_GET['email'] ) ) {
					...
					if ( version_compare( get_woocommerce_version(), '2.4', '>=') ) {
						/* 2.4 and newer */
						new WWPDF_File_Handler();
					} else if ( version_compare( get_woocommerce_version(), '2.3', '>=') ) {
						/* 2.3 and newer */
                        remove_action( 'init', array( 'WC_Download_Handler', 'download_product') );
						WWPDF_Download_Handler_23::init();
					} else if ( version_compare( get_woocommerce_version(), '2.3', '<') ) {
						/* 2.2.11 and older */
    					remove_action( 'init', array( 'WC_Download_Handler', 'download_product') );
						new WWPDF_Download_Handler_22();
					}
				...
				e_( 'Yikes! Your WooCommerce plugin installation seems to be missing the WC_Download_Handler class.' );
	...
	'<p>' . __( '<a href="http://waterwoo.me/buy/waterwoo-premium/" target="_blank">Consider upgrading to the Premium version</a> if you need more functionality. The premium version adds the ability to start watermarking on a specified page, password protect and copy/print/modify protect your document, a watermark overlay, HTML input, and UTF-8 and extended Unicode support for many more foreign language characters. Thanks again, and enjoy!', 'waterwoo-pdf' ) . '</p>'
			...
			$screen->set_help_sidebar(
				...
				'<p><a href="http://waterwoo.me/buy/waterwoo-premium/" target="_blank">' . __( 'Upgrade to Premium', 'waterwoo-pdf' ) . '</a></p>'
		...
		public function upgrade_to_premium_link( $links ) {
			...
			if ( function_exists( 'is_plugin_active' ) && ! is_plugin_active( 'waterwoo-pdf-premium/waterwoo-pdf-premium.php' ) ) {
				...
				$links[] = '<a href="http://waterwoo.me/buy/waterwoo-premium/" target="_blank">' . __( 'Upgrade to Premium', 'waterwoo-pdf' ) . '</a>';
		...
		public function get_premium_cta() {
			...
			if ( function_exists( 'is_plugin_active' ) && ! is_plugin_active( 'waterwoo-pdf-premium/waterwoo-pdf-premium.php' ) ) {
				...
				$link = 'http://waterwoo.me/buy/waterwoo-premium/';
		...
		public function wwpdf_admin_notices() {
			...
			if ( ! get_user_meta($user_id, 'wwpdf_ignore_notice7') ) {
				$currentscreen = get_current_screen();
				if ( $pagenow == 'admin.php' && $currentscreen->id == 'woocommerce_page_wc-settings' ) {
					echo '<div class="updated"><p>'; 
					printf(__('<p>Encourage Wordpress plugin authors to keep making & supporting free plugins! :) If you like WaterWoo, please <a href="https://wordpress.org/support/view/plugin-reviews/waterwoo-pdf?filter=5" target="_blank">submit a kind review</a> or <a href="https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=PB2CFX8H4V49L" target="_blank">make a small donation</a>. <strong>Thank you</strong> for using WaterWoo!</p><p><a href="%1$s">Hide This Notice</a></p>'), '?page=wc-settings&tab=waterwoo-pdf&nag_ignore=0');
					echo "</p></div>";
				}
		...
		public function nag_ignore() {
			...
			if ( isset($_GET['nag_ignore']) && '0' == $_GET['nag_ignore'] ) {
				...
				delete_user_meta($user_id, 'wwpdf_ignore_notice4', 'true', true);
				delete_user_meta($user_id, 'wwpdf_ignore_notice5', 'true', true);
				delete_user_meta($user_id, 'wwpdf_ignore_notice6', 'true', true);
				add_user_meta($user_id, 'wwpdf_ignore_notice7', 'true', true);
			}
		}
		...
		private function settings_array() {
			...
			return array(
				...
				array(		
					...
					'desc' 	=> __( 'Move the footer watermark up and down on the page by adjusting this number. Default is 270 millimeters (bottom of letter-sized page).', 'waterwoo-pdf' ),
